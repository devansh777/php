...
function getFieldValue($fieldname,$table,$returntype=""){   
    ...
    if($table == 'post_category'){
        ...
        foreach ($data as $key => $value){
            array_push($abc,$value[1]);
        }
        return (!empty($abc) ? $abc : $returntype);     
    ...
    function filterblog($post)
    {
        ...
        if(isset($_POST['updateblog'])){
            ...
            $input['post_id'] = $_SESSION['editid']; 
            updatedata('post_category',$post_ID,$_SESSION['editid'],'post_id');
        }
        else{
            $data['created_at'] = date('m/d/Y h:i:s a', time());
            insertdata($table,$data);
